...
render :: State -> VNode
render s = div (cls "state") [ch|button', pixelDiv,numDiv|]
    where
      button'  = textDiv (y s)
      xd       = textDiv (y s)
      yd       = textDiv (x s)
...
animate :: DOMNode -> VNode -> State -> (MVar State) -> IO ()
animate n r s q =
  let s' = step s
      r' = render s'
      p  = diff r r'
--  in  s' `seq` redraw n p >> threadDelay 20000 >> animate n r' s' -- for async calculation, sync repaint
  in do
    ms <- tryTakeMVar q
    case ms of
      (Just s0) -> atAnimationFrame (patch n p >> animate n r' s0 q) -- sync all
      Nothing -> atAnimationFrame (patch n p >> animate n r' s' q) -- sync all
